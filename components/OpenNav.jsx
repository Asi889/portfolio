...
const OpenNav = (props) => {
  ...
  return (
    <div
      className={` railway relative ${
        theme.darkTheme ? "bg-[#363030]" : "bg-[#757373]"
      } md:grid block text-white z-40 px-4 h-full   transition duration-[2s] ease-in-out ${
        isOpen
    ...
    >
        ...
        {navLinks.map((nav) => {
          return (
            <Link
              ...
              href={nav.to}
              key={nav.to}
              className={`nav-link italic border-b-2  ${
                theme.darkTheme
      ...
      <div
        className={`open-nav-link-wrapper ${
          theme.darkTheme ? "text-white" : "text-[#535252]"
        } z-50 flex flex-wrap md:gap-x-10 gap-x-6 md:pt-40 pt-24 justify-center mix-blend-difference ${
          isOpen ? "" : "hidden"
        }`}
      >
        <div
