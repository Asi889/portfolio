...
//       </div>
//         <div className="flex">All rights reserved to Asaf Marom © Date.now()</div>
  ...
  return (
    <div
      className={` railway relative ${theme.darkTheme ? "bg-[#363030]" : "bg-[#757373]"
        } md:grid block text-white z-40 px-4 h-full  transition duration-[2s] ease-in-out ${isOpen
          ? " w-full text-center  justify-center   justify-items-center"
          : "w-0"
        }`}
    >
      <div className="nav-links-wrapper self-end  mix-blend-difference lg:pt-10 pt-4 z-20 md:flex grid flex-wrap h-fit justify-center font-extrabold md:text-5xl text-4xl gap-x-10 md:gap-y-4 gap-y-8  transition-all duration-500 ease-in-out">
        {navLinks.map((nav) => {
          return (
            <Link
              ...
              href={nav.to}
              key={nav.to}
              className={`nav-link italic border-b-2  ${theme.darkTheme
                ? "text-[#fff] border-white"
                : "text-[#727072] border-[#727072]"
                }   h-fit z-50   `}
      ...
      <div
        className={`open-nav-link-wrapper ${theme.darkTheme ? "text-white" : "text-[#535252]"
          } z-50 grid md:gap-x-10 gap-x-6 md:pt-12 pt-20 justify-center mix-blend-difference h-full ${isOpen ? "" : "hidden"
          }`}
      >
        <div className="grid h-full ">
          <div className="flex gap-x-6 flex-wrap justify-center">
            ...
            <div
              onMouseEnter={textEnter}
              onMouseLeave={textLeave}
              className="grid justify-center gap-y-4 justify-items-center  h-fit"
            >
              <GithubIcon />
              <a
                target="_blank"
        ...
        <div className="flex mix-blend-difference lg:pt-0 pt-4 self-end pb-2 justify-center">All rights reserved to Asaf Marom © </div>
      </div>
